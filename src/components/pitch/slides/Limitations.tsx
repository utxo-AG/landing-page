    ...
    <SlideWrapper variant="cool">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        <motion.div variants={itemVariants}>
          <p className="text-xs font-mono text-[#999] uppercase tracking-wider mb-4">{t("cannotLabel")}</p>
          <div className="space-y-3">
            {LIMITATIONS.filter((l) => l.type === "cannot").map((item) => (
              <div key={item.key} className="flex items-start gap-3">
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#ddd] text-[#888] text-xs flex-shrink-0 mt-0.5">✕</span>
        ...
        <motion.div variants={itemVariants}>
          <p className="text-xs font-mono text-[#999] uppercase tracking-wider mb-4">{t("bestAtLabel")}</p>
          <div className="space-y-3">
