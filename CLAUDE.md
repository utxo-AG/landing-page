...
- **Language switch**: the inline head script is identical on all pages and reads the hreflang links. A stored manual choice (`localStorage.utxo_lang`, set by `[data-set-lang]` links) always wins. Without one, only English pages redirect to German when the browser language is German. German pages never auto-redirect, otherwise Googlebot (crawls as en-US) never sees them.
